...
internal static partial class PersonalAgentEndpoints
{
    public static WebApplication MapPersonalAgentEndpoints(this WebApplication app)
    {
        ...
        apiGroup.MapGet("/coach-checkins/{uploadId:guid}/transcript.txt", async (HttpContext httpContext, Guid uploadId, string? profileId, AgentService agentService, ICoachAssignmentStore assignmentStore) =>
        {
            ...
            var content = string.Join(Environment.NewLine, transcript.Utterances.Select(utterance => $"[{TimeSpan.FromMilliseconds(utterance.StartMs):hh\\:mm\\:ss}-{TimeSpan.FromMilliseconds(utterance.EndMs):hh\\:mm\\:ss}] {utterance.SpeakerName} ({utterance.SpeakerRole}): {utterance.Text}"));
