...
internal partial class AgentChatService
{
    ...
    public Task<string?> SendMessageAsync(string sessionId, AgentAccessContext access, string message, CancellationToken cancellationToken = default)
        => PersonalAgent.Integrations.AiTelemetry.RunAsync("agent.run", "AGENT",
        ...
        Func<string, CancellationToken, ValueTask>? OnDelta, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var Presentation = new PersonalAgent.Contracts.ChatPresentation { Sources = Context?.Sources ?? [] };
            if (sessionState.IsContinuous)
            {
                var Parsed = PersonalAgent.Contracts.ChatCardParser.Parse(responseText);
