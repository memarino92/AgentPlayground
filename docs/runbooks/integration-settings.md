...
3. Sign in normally and open **Settings** in the menu (`/admin/settings`). Sentry, OpenTelemetry, AI providers, and existing database settings are sections on this one page. The old `/admin/integration-settings` URL opens the same page. The synthetic Compose environment already permits `demo-owner`; `demo-other` is deliberately denied.

## AI provider setup

Open **AI providers** to create or replace the OpenRouter API credential. This dedicated form is the supported first-time setup path: the server fixes the destination to the active, encrypted `Api` / `OpenRouter:ApiKey` row, so the browser cannot choose an arbitrary secret key or scope. The current secret is never returned. Creation and replacement use the row version to reject concurrent changes, and a successful save triggers an immediate API credential check. If `OPENROUTER_API_KEY` is set in deployment configuration, the page reports that the override remains effective without revealing its value.

Once created, the row also appears in **Database settings**, where it can be deactivated. OpenRouter model eligibility remains separate under `ChatModels:*`; see [runtime chat models](runtime-chat-models.md).
...
OpenAI, OpenRouter, and AssemblyAI API keys in Shared/Api scopes reload automatically in API within 15 seconds. **Reload supported credentials in API** performs an immediate check and shows its result plus active environment override names. API-specific entries override Shared entries; nonempty `OPENAI_API_KEY`, `OPENROUTER_API_KEY`, and `ASSEMBLYAI_API_KEY` environment variables win. Supported key replacements must be nonempty printable tokens of at most 4096 characters. Invalid external edits or read/decryption failures retain the current credential snapshot. Provider acceptance is not checked; a syntactically valid but revoked key can still fail requests. Rotate within the same account when outstanding provider job identifiers must remain accessible.

New chat, embedding, journal parsing, model discovery and transcription provider requests use the accepted credential. Existing HTTP requests retain their original client. Chat model policy under Shared/Api `ChatModels:*` is read directly from the database on each catalog request; reload the chat page after editing it. No API restart or credential reload is needed for those edits. See [runtime chat models](runtime-chat-models.md). Other settings still require restarting affected services through the deployment platform. Shared settings may require restarting API, Web and Worker. Coordinate matching internal API and actor-signing keys before restarting services; verify sign-in and service connectivity afterward. A browser refresh or Sentry reload does not reload these other settings. The page does not claim every saved value is running. `DATABASE_URL` and `CONFIG_ENCRYPTION_KEY` remain external bootstrap configuration.

The generic database editor updates existing rows in place; it does not create/delete keys, change encryption flags, retain immutable history, or provide rollback. Provider-specific setup forms may create narrowly whitelisted rows with server-owned scope, key, and secret metadata; OpenRouter is the first such form. Keep database recovery procedures available before rotating credentials. Sentry's revision history and live application behavior below remain separate.
