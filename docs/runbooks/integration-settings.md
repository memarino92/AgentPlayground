...
OpenAI and AssemblyAI API keys in Shared/Api scopes now reload automatically in API within 15 seconds. **Reload supported credentials in API** performs an immediate check and shows its result plus active environment override names. API-specific entries override Shared entries; nonempty `OPENAI_API_KEY` and `ASSEMBLYAI_API_KEY` environment variables win. Supported key replacements must be nonempty printable tokens of at most 4096 characters. Invalid external edits or read/decryption failures retain the current credential snapshot. Provider acceptance is not checked; a syntactically valid but revoked key can still fail requests. Rotate within the same account so outstanding transcription jobs remain accessible.
