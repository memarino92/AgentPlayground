...
On the first API startup containing the Auto-default migration, the policy gains `openrouter:openrouter/auto` and its display name at an unused index, earlier defaults become false, and `ChatModels:OpenRouterAutoDefaultInitialized` records completion. If an Auto entry already exists, its index is reused. The migration supports current and older settings-table shapes and runs in the existing policy transaction. Later administrator edits remain authoritative because the marker makes subsequent startups no-ops.

The catalog queries configured providers independently. One provider's discovery failure does not remove healthy choices from another; if Auto is not returned by discovery, the first available reviewed model becomes the effective default. OpenRouter Auto performs prompt-based model selection and provider routing. The actual downstream model can change as OpenRouter's pool changes, so using it trades model pinning for automatic routing. OpenRouter documents implicit conversation stickiness derived from the first system and user messages; this integration does not yet send an explicit `session_id`, constrain the router's allowed model pool, expose routed-model metadata in the UI, or enforce a cost tier.
...
Edit policy in **Settings → Database settings**, filtering for `ChatModels`. Each indexed model has `Models:N:Id`, `Models:N:DisplayName`, and `Models:N:IsDefault` entries. Change an existing ID/label to another reviewed model, or deactivate its ID row to remove that choice. An active Shared ID may still supply the value when an Api ID is inactive. The migration creates the Auto slot; adding other slots still requires an authorized database migration because the generic editor only edits existing rows. No model choices are hardcoded in Web.
