...
- Audio is staged as database bytes; API retrieves it for transcription; `TranscribeCoachCallConsumer` receives neutral segments, persists utterances, and waits for speaker attribution when required. See [transcription operations](runbooks/transcription-gateway.md).
