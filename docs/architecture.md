...
`PersonalAgent.Integrations` is host infrastructure shared by API/Web/Worker for encrypted runtime integration configuration, metadata-only Sentry error reporting, OpenTelemetry instrumentation and OpenInference AI spans. Web provides the settings form; API authorizes administrators and owns revision edits/promotions; each host independently reconciles active Sentry and telemetry revisions. Export is disabled until configured. See [0010](decisions/0010-runtime-integration-settings.md), [0019](decisions/0019-observability.md) and [0020](decisions/0020-live-database-telemetry-settings.md).

## Current persistence and operations

- Agent-authored deterministic automations use an API-hosted MassTransit 8.5.8 saga, EF Core/Npgsql persistence and transactional outboxes in the dedicated `automation` schema. Chat creates immutable registered-action recipe revisions. Runs pin source versions; result transitions atomically commit reports, progress and outgoing messages. A separate automation dashboard exposes schedules, revisions and outcomes. Existing scheduled chat jobs and transcription remain on their existing storage/execution paths. See [automation operations](runbooks/automations.md).
- Optional C# steps dispatch through the EF outbox to a trusted AutomationRunner service on Railway. Each run gets an ISOLATED Railway VM with Internet access and a bounded nonroot compiler container. Durable leases track VM cleanup and prevent automatic replay of uncertain executions. A run-scoped capability gateway exposes declared, currently authorized read tools; managed packages require exact locked dependencies and administrator approval. Optional Jev review adds a further check. Domain writes remain later saga/outbox steps. The dashboard retains source/image fingerprints, diagnostics, operation decisions and sandbox cleanup status. A guarded offline Docker backend supports local synthetic tests. See [program operations](runbooks/automation-programs.md).
...
- Scheduled agent tasks and notifications have durable job IDs, trusted actor/subject attribution, attempts, current authorization checks, guarded pending cancellation, and optional fixed-interval recurrence. API owns execution and advances successful series; Worker delivers job IDs. Interrupted agent runs recover a saved response or enter NeedsReview instead of blindly replaying effects. Completion intent uses the domain outbox. A restored database can contain pending real actions. See [scheduled jobs](runbooks/scheduled-jobs.md).
