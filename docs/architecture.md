...
- Optional C# steps dispatch through the EF outbox to a dedicated trusted AutomationRunner. Each source file compiles/runs in a disposable, nonroot, network-disabled and resource-limited container with explicit stdin only; results return to the saga for authorization and domain commits. The API never receives a Docker socket. Per-run source/compiler fingerprints and diagnostics are retained with step history. See [program operations](runbooks/automation-programs.md).
