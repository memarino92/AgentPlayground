...
| --- | --- | --- |
| CONFIG-01 + OPS-01 | In progress — Sentry and existing settings editor implemented | In-app Sentry settings and metadata-only error reporting pass local API/Web/Worker and synthetic checks; maintainer confirmed deployed test-event receipt. All existing database settings are editable with restart requirements. Broader history UI, typed legacy validation, tracing, and alerts remain open. |
...
| DEBT-08 | **Shared infrastructure and Web dependencies:** separate neutral wire contracts from PostgreSQL configuration, transport bootstrap, and outbox implementation. Verify whether Web needs a bus and remove its bus hosting if unused. | Contract consumers do not acquire database/transport implementation dependencies merely to use DTOs; Web authentication, API calls, Data Protection persistence, and startup still work without unnecessary messaging infrastructure. | Refines DATA-06; preserve required Web database access and deployed configuration. Inspect actual callers before removing registrations; do not introduce another service or broker. |
...
There are **54 selectable items**. Useful combinations for a future prioritization conversation are:
