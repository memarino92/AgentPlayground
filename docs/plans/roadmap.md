...
Research and planning only; no provider integration or production activation. The [implementation plan](jev-integration.md) covers two first-class experiments: pre-chat tool routing/general automation with simulated home devices, and coaching-evidence reranking. Test direct commands, tool suggestions, clarification and main-chat fallback, with independent promotion gates. Semantic tagging remains a follow-up. See [proposed decision 0026](../decisions/0026-structured-decision-provider.md). This extends the proposed scope without marking MEMORY-02 or LAB-01/05 complete or adding a delivered backlog item.

**Deliver:** verify the synthetic contract and account terms; implement neutral API capabilities with encrypted settings and independent mode switches; reuse registry/binder authorization for tool routing; add simulated home tools, durable turn/action identity and uncertain-outcome recovery before direct writes. Run paired automation and retrieval evaluations; select a real home platform separately. Preserve normal chat, stereo attribution, citations, subject isolation and recording scope.

**Done when:** each capability meets its own frozen held-out quality/latency/cost gates; automation tests verify full arguments, resulting device state, no unintended actions and no blind replay after ambiguous outcomes; current chat/retrieval/security regressions pass. Provider outage and invalid settings preserve baseline behavior; default Off and rollback are verified. Private-data terms are resolved before private trials. Record actual evidence here when implemented; simulated devices do not establish real-home readiness.
