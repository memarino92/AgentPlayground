...
Research and planning only; no provider integration or production activation. The [implementation plan](jev-integration.md) covers pre-chat routing, LLM-callable structured decisions and delegated tool dispatch through deterministic binding, pre/post-call judging, and evidence reranking. Test with simulated home devices and independent promotion gates. Semantic tagging remains a follow-up. See [proposed decision 0026](../decisions/0026-structured-decision-provider.md). This extends the proposed scope without marking MEMORY-02 or LAB-01/05 complete or adding a delivered backlog item.
