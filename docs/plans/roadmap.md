...
Follow-up in PR #73: default-disabled, owner-only C# execution in Railway Sandboxes, Internet enabled, immutable compiler image, durable cleanup/replay records, scoped read-tool gateway, locked/allowlisted packages and optional Jev reviews. Encrypted setup UI and dashboard operation/sandbox evidence are implemented. PostgreSQL/fake-provider tests and local Docker checks cover boundaries; live Railway lifecycle and Jev review quality remain acceptance checks before production enablement. See [decision 0036](../decisions/0036-railway-automation-sandboxes.md) and [operations](../runbooks/automation-programs.md). Future work: automatic lock generation, human approval queues, artifact caching and optional egress proxy. Existing services remain outside the saga migration scope.
...
Per-file Git blob checkpoints now skip downloads, parsing, and embedding requests for unchanged journal files. Changed files retain entry-level embedding reuse; checkpoints commit with entry writes, and failures remain eligible for retry. The first sync establishes checkpoints. See [decision 0028](../decisions/0028-journal-file-checkpoints.md) and the [reprocessing runbook](../runbooks/journal-sync.md).

Validation: all 18 targeted journal tests passed on 2026-09-20, including PostgreSQL tests for unchanged/changed files, source scoping, empty results, missing metadata, failure retries, and rollback after partial writes, plus known Git blob hashes and request timeouts. This does not implement deletion reconciliation or concurrent-sync serialization.

## Jev structured decisions — first routing slice implemented 2026-09-19

The first API-owned Choice adapter and pre-chat router are implemented, default Off: authorized local tool suggestions, bounded shadow calls, a narrow read-only clock fast path, encrypted database settings with validated live reload, synthetic provider wiring, and metadata-only Sentry/OpenInference instrumentation. Production clock tests now confirm successful provider calls and direct execution without chat inference. The expanded router covers all 16 current local/Tavily tools when authorized, with application-owned remote-tool metadata, routing outcome logs and opt-in DirectTools adapters for explicit commands; 308 API tests passed at the original delivery point. See [decision 0027](../decisions/0027-full-tool-routing.md). Live expanded-catalog accuracy, notification delivery and quality/latency/cost comparisons remain outstanding. See [operations and limits](../runbooks/jev-routing.md). The broader [implementation plan](jev-integration.md) still proposes simulated home devices, delegated decisions/actions, independent judging and evidence reranking. Semantic tagging remains a follow-up. [Decision 0026](../decisions/0026-structured-decision-provider.md) accepts this bounded first slice; the broader experiments remain proposed. MEMORY-02 and LAB-01/05 are not complete. Recovery rehearsal is deferred at the maintainer's request.

Validation on 2026-09-19: initially 184 API tests without PostgreSQL dependencies (including 56 new Jev cases) and all 56 Web tests passed. After Docker became available, all 276 API tests passed, including the Jev PostgreSQL settings test; rebuilt server containers were healthy and all 22 synthetic smoke checks passed. Browser checks verified the key control, saving routing mode, a direct clock response and persisted conversation after reload. Coverage includes chat persistence/fallback, skipping chat/embedding calls for direct responses, authorization revocation, HTTP contract bounds, cancellation, throttling and Sentry correlation/redaction. No measured model-quality improvement is claimed.

**Deliver:** implement neutral API capabilities with encrypted settings and independent switches; reuse registry/binder authorization; add bounded decision/delegation tools and a typed invocation compiler; prevent leaf-tool bypass in strict delegated mode. Add simulated devices, durable action identity and uncertain-outcome recovery. Evaluate judging in Observe mode before enforcement, including ordinary LLM calls and Jev-selected calls. Compare against schema-constrained LLM calls plus deterministic validation. Select a real home platform separately; preserve existing access and retrieval boundaries.

**Done when:** each capability meets frozen held-out gates; no invalid requests reach enrolled handlers; exact action correctness and supported-request coverage are measured separately from schema validity. Independent labels establish judge error rates; no bypass, duplicate effects or blind replay occurs. Existing regressions pass, default Off and rollback work, and Enforce-mode outages block enrolled mutations instead of bypassing the judge. Private-data terms precede private trials. Record actual evidence when implemented; simulated devices do not establish real-home readiness.
...
2026-09-12: TRUST-01 and the conservative scheduled-job slice of DEBT-01 are implemented with a FLOW-02 dashboard and pending cancellation. API persists trusted actor/subject/source conversation, checks current database sign-in policy, assignments and tool permissions, owns execution by TaskId, and commits completion/notification intent through the domain outbox. Interrupted runs recover saved responses or become NeedsReview without blind replay. Legacy deliveries are blocked. See [decision 0021](../decisions/0021-scheduled-jobs.md) and [operations](../runbooks/scheduled-jobs.md). General workflow resume, external-tool operation journals and other job types remain open.

2026-09-24: the first FLOW-06 slice adds agent-created fixed-interval recurrence to tasks and notifications. Successful occurrences retain attempts and advance one stable series to its next future run; authorization failures and ambiguous outcomes stop it. The dashboard shows recurrence and next-run state and cancels the full series. Calendar/cron rules, wall-clock DST behavior, pause/resume, end conditions and explicit missed-run controls remain open. See [decision 0034](../decisions/0033-recurring-scheduled-jobs.md).
...
| FLOW-06 | In progress — fixed-interval recurrence delivered | Agent-created task and notification series, run history, next-run visibility, skipped missed intervals and series cancellation are implemented. Calendar rules, pause/resume, end conditions and wall-clock/DST semantics remain open. Decision 0033. |
...
- API-local discovery and catalog interfaces separate provider inventory from application selection policy. OpenAI and optional OpenRouter adapters merge independently discovered inventories; clients keep the existing contract. OpenRouter Auto is the provider-managed default for new chats after a one-time existing-policy migration, while Jev remains focused on structured tool decisions. Settings can create or replace the encrypted OpenRouter credential and trigger live reload without a pre-existing database row. See [decision 0032](../decisions/0032-openrouter-auto-routing.md).
...
- Added `PersonalAgent.Integrations`, a dedicated host-infrastructure library with encrypted immutable configuration revisions, optimistic concurrency, promotion history, an additive versioned schema migration, and per-instance acknowledgement records.
