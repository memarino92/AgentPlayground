...
`/admin/settings` edits the existing table; `/admin/integration-settings` retains Sentry controls. PostgreSQL tests cover production and older synthetic schemas, encryption, secret retention/clearing, custom and inactive rows, external-write conflicts and batch rollback. API tests cover unauthorized actors; Web tests cover edits across filters, secret cleanup and conflict feedback. See [operations](../runbooks/integration-settings.md) and `CONFIG-01` in the [roadmap](../plans/roadmap.md).
